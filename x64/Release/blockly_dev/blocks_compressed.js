...
Blockly.Blocks.lists_getIndex = {
	init: function() {
		...
		this.setTooltip(function() {
			...
			if ("FROM_START" == d || "FROM_END" == d) e += "  " + ("FROM_START" == d ? Blockly.Msg.LISTS_INDEX_FROM_START_TOOLTIP :
...
Blockly.Blocks.lists_setIndex = {
	init: function() {
		...
		this.setTooltip(function() {
			...
			if ("FROM_START" == d || "FROM_END" == d) e += "  " + Blockly.Msg.LISTS_INDEX_FROM_START_TOOLTIP.replace("%1", b
...
Blockly.Constants.Logic.CONTROLS_IF_MUTATOR_MIXIN = {
	...
	reconnectChildBlocks_: function(a, b, c) {
		for (var d = 1; d <= this.elseifCount_; d++) Blockly.Mutator.reconnect(a[d], this, "IF" + d), Blockly.Mutator.reconnect(
			b[d], this, "DO" + d);
...
Blockly.Constants.Logic.LOGIC_TERNARY_ONCHANGE_MIXIN = {
	...
	onchange: function(a) {
		...
		if ((b || c) && d)
			for (var e = 0; 2 > e; e++) {
				var f = 1 == e ? b : c;
				f && !f.outputConnection.checkType(d) && (Blockly.Events.setGroup(a.group), d === this.prevParentConnection_ ? (
					this.unplug(), d.getSourceBlock().bumpNeighbours()) : (f.unplug(), f.bumpNeighbours()), Blockly.Events.setGroup(
					!1))
			}
...
Blockly.Constants.Loops.CONTROL_FLOW_IN_LOOP_CHECK_MIXIN = {
	...
	onchange: function(a) {
		if (this.workspace.isDragging && !this.workspace.isDragging() && a.type == Blockly.Events.BLOCK_MOVE && a.blockId ==
			this.id) {
			var b =
...
Blockly.defineBlocksWithJsonArray([{
	...
	type: "math_constrain",
	message0: "约束 %1 介于(最小值) %2 和(最大值) %3 ",
	args0: [{
		type: "input_value",
		name: "VALUE",
		check: "Number"
	...
	"output": null,
	"colour":"#44D484",
	"tooltip": "将一个数从第一个区间映射到第二个区间，并返回映射之后的值。",
	"helpUrl": ""
}, {
...
Blockly.Blocks.procedures_defnoreturn = {
		...
		for (var b = 0, c; c = a.childNodes[b]; b++)
			if ("arg" == c.nodeName.toLowerCase()) {
				...
				c = Blockly.Variables.getOrCreateVariablePackage(this.workspace, c,
					d, "");
				null != c ? this.argumentVarModels_.push(c) : console.log("Failed to create a variable with name " + d +
					", ignoring.")
			} this.updateParams_();
	...
	renameVarById: function(a, b) {
		...
		if ("" == c.type) {
			...
			for (var d = !1, e = 0; e < this.argumentVarModels_.length; e++) this.argumentVarModels_[e].getId() == a && (this.arguments_[
				e] = b.name, this.argumentVarModels_[e] = b, d = !0);
	...
	displayRenamedVar_: function(a, b) {
		...
		if (this.mutator && this.mutator.isVisible())
			for (var c = this.mutator.workspace_.getAllBlocks(!1), d = 0, e; e = c[d]; d++) "procedures_mutatorarg" == e.type &&
				Blockly.Names.equals(a, e.getFieldValue("NAME")) && e.setFieldValue(b, "NAME")
	},
	customContextMenu: function(a) {
		if (!this.isInFlyout) {
			var b = {
			...
			if (!this.isCollapsed())
				for (c = 0; c < this.argumentVarModels_.length; c++) b = {
					...
					d = Blockly.Variables.generateVariableFieldDom(d), e = Blockly.utils.xml.createElement("block"), e.setAttribute(
						"type", "variables_get"), e.appendChild(d), b.callback = Blockly.ContextMenu.callbackFactory(this, e), a.push(b)
		}
	},
...
Blockly.Blocks.procedures_callnoreturn = {
		...
		if (b)
			if (a.join("\n") == this.arguments_.join("\n")) this.quarkIds_ = b;
				...
				for (var e = 0; e < this.arguments_.length; e++) {
					...
					f && (f = f.connection.targetConnection, this.quarkConnections_[this.quarkIds_[e]] = f, d && f && -1 == b.indexOf(
						this.quarkIds_[e]) && (f.disconnect(), f.getSourceBlock().bumpNeighbours()))
				}
				...
				this.updateShape_();
				if (this.quarkIds_ = b)
					for (e = 0; e < this.arguments_.length; e++) b = this.quarkIds_[e], b in this.quarkConnections_ && (f = this.quarkConnections_[
						b], Blockly.Mutator.reconnect(f, this, "ARG" + e) || delete this.quarkConnections_[b]);
	...
	domToMutation: function(a) {
		...
		b = [];
		for (var c = [], d = 0, e; e = a.childNodes[d]; d++) "arg" == e.nodeName.toLowerCase() && (b.push(e.getAttribute(
			"name")), c.push(e.getAttribute("paramId")));
