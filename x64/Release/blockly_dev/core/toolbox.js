...
Blockly.Toolbox.prototype.handleBeforeTreeSelected_ = function(node) {
  ...
  if (this.lastCategory_) {
    var sdi =(this.lastCategory_.id_);
    // this.lastCategory_.getRowElement().style.backgroundImage = '';   
   var imgName = document.getElementById(sdi).getAttribute("aria-posinset");
   var imgName = imgName -1;
    var hexhive = 'url(../static/images/home' + imgName +'.svg)';
    this.lastCategory_.getRowElement().style.backgroundImage= hexhive;
  ...
  if (node) {
    var sdi =node.id_;
    var imgName = document.getElementById(sdi).getAttribute("aria-posinset");
    var hexhive = 'url(../static/images/home_cli' + imgName +'.svg)';
    node.getRowElement().style.backgroundImage = hexhive;
    // Add colours to child nodes which may have been collapsed and thus
    // not rendered.
    this.addColour_(node);
  }
...
Blockly.Toolbox.prototype.updateSelectedItemColour_ = function(tree) {
  ...
  if (selectedItem) {
    var hexColour = selectedItem.hexColour || ' ';
  ...
  for (var i = 0, child; child = children[i]; i++) {
    ...
    if (element) {
      if (this.hasColours_) {
        ...
        var bgim = 'url(../static/images/home' +i+'.svg)';
        var rep = 'no-repeat';
      } else {
        var border = 'none';
      }
      if (this.workspace_.RTL) {
        ...
        element.style.backgroundImage = bgim;
        element.style.backgroundRepeat = rep;
